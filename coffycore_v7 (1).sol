...
// ╔══════════════════════════════════════════════════════════════════════════════╗
// ║  COFFY COIN — IMMORTAL CORE V7                                               ║
// ║  Base (Coinbase L2)  |  Immutable Core Token                                ║
// ║  ⚠️  THIS CONTRACT CANNOT BE CHANGED AFTER DEPLOY                               ║
// ╚══════════════════════════════════════════════════════════════════════════════╝
//
// FIXES (V6 → V7):
//  [V7-FIX-01] moduleRegistered mapping — O(1) duplicate check replaces O(n) for-loop in registerModule.
//              moduleList.push guard loop removed. Gas savings on every registerModule call.
//  [V7-FIX-02] setMultisig(newAddr) — only current multisig can call. Old multisig's ADMIN_ROLE
//              and DEFAULT_ADMIN_ROLE are automatically revoked. Prevents multisig lock-in.
//  [V7-FIX-03] moduleTransferLimit mapping + setModuleLimit() — per-module weekly spend cap on
//              payRewardFromModule. game: 15M, activity: 1M recommended. Prevents a single module
//              from consuming the entire weekly community pool.
//  [V7-FIX-04] setTimelockModule auto-revoke — when a new timelock is set, old timelock's
//              TIMELOCK_ROLE is automatically revoked. Prevents orphan timelock privilege.
//
// FIXES (V5 → V6):
...
contract CoffyCoin is ERC20, AccessControl, ReentrancyGuard, Pausable {
    ...
    // [V7-FIX-01] O(1) duplicate check — replaces O(n) for-loop in registerModule
    mapping(address => bool) public moduleRegistered;

    // [V7-FIX-03] Per-module weekly spend limit on payRewardFromModule (0 = unlimited)
    mapping(address => uint256) public moduleTransferLimit;
    // Weekly tracker: weekKey => module => amount sent
    mapping(uint256 => mapping(address => uint256)) public moduleWeeklySent;
    ...
    event MultisigChanged(address indexed oldMultisig, address indexed newMultisig); // [V7-FIX-02]
    event ModuleLimitSet(address indexed module, uint256 limit); // [V7-FIX-03]
    ...
    function registerModule(address _module, string calldata _name, string calldata _kind) external onlyAdmin {
        ...
        // [V7-FIX-01] O(1) duplicate check — no for-loop needed
        if (moduleRegistered[_module]) revert ModuleAlreadyRegistered();
        ...
        // [V7-FIX-01] O(1) guard — for-loop removed
    ...
    function setAuthorizedModule(address module, bool authorized) external onlyActiveAdmin {
        if (authorized) {
            if (modules[module].status == 0) { // If not registered, register it as generic
                ...
                // [V7-FIX-01] moduleRegistered sync — legacy path must also set this
                if (!moduleRegistered[module]) {
                    moduleRegistered[module] = true;
    ...
    // [V7-FIX-04] setTimelockModule — old timelock's TIMELOCK_ROLE auto-revoked
    function setTimelockModule(address m) external onlyActiveAdmin {
        if (m == address(0)) revert ZeroAddress();
        address old = timelockModule;
        // Auto-revoke old timelock role
        if (old != address(0) && hasRole(TIMELOCK_ROLE, old)) {
            _revokeRole(TIMELOCK_ROLE, old);
        }
        timelockModule = m;
        _grantRole(TIMELOCK_ROLE, m);
    ...
    // [V7-FIX-03] Per-module limit check added
    function payRewardFromModule(address _to, uint256 _amt) external onlyActiveModule nonReentrant whenNotPaused {
        ...
        // [V7-FIX-03] Per-module weekly limit (0 = unlimited)
        if (moduleTransferLimit[msg.sender] > 0) {
            if (moduleWeeklySent[wk][msg.sender] + _amt > moduleTransferLimit[msg.sender]) revert WeeklyLimitExceeded(); // MODULE_WEEKLY_LIMIT
            moduleWeeklySent[wk][msg.sender] += _amt;
        }
    ...
    function executeRecovery() external {
        ...
        // [V7-BUGFIX] multisig state updated — onlyMultisig modifiers work after recovery
        multisig = recoveryAddress;
    ...
    // [V7-FIX-02] setMultisig — only current multisig can call, old roles auto-revoked
    function setMultisig(address newAddr) external onlyMultisig {
        if (newAddr == address(0)) revert ZeroAddress();
        if (newAddr == multisig) revert InvalidParameter();
        address old = multisig;
        // Auto-revoke old multisig roles
        if (hasRole(ADMIN_ROLE, old))         _revokeRole(ADMIN_ROLE, old);
        if (hasRole(DEFAULT_ADMIN_ROLE, old)) _revokeRole(DEFAULT_ADMIN_ROLE, old);
        multisig = newAddr;
        _grantRole(DEFAULT_ADMIN_ROLE, newAddr);
        _grantRole(ADMIN_ROLE, newAddr);
        emit MultisigChanged(old, newAddr);
    }

    // [V7-FIX-03] setModuleLimit — set per-module weekly reward cap (0 = unlimited)
    // Recommended: game = 15_000_000e18, activity = 1_000_000e18
    function setModuleLimit(address module, uint256 limit) external onlyTimelockOrAdmin {
        if (!moduleRegistered[module]) revert ModuleInactive(); // NOT_REGISTERED
        moduleTransferLimit[module] = limit;
        emit ModuleLimitSet(module, limit);
    }
