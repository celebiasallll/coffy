...
let provider;
let moduleContract;
let signerContract; // Signer-connected contract for on-chain calls (emergencyCancelGame etc.)
...
async function handleGameEnd(roomId, winner, reason) {
    ...
    // Debug file logger for draw troubleshooting
    const logMsg = (msg) => {
        const ts = new Date().toISOString();
        const line = `[${ts}] ${msg}\n`;
        console.log(msg);
        try { fs.appendFileSync(join(__dirname, 'game_debug.log'), line); } catch (_) { }
    };

    logMsg(`🏁 Game ended in ${roomId}: winner="${winner}" reason="${reason}" gameId=${room.meta?.gameId} signerContract=${!!signerContract}`);
    ...
    try {
        if (!process.env.SIGNER_PRIVATE_KEY) {
            ...
            // ── DRAW: Server calls emergencyCancelGame to refund both players ──
            if (winner === 'draw') {
                if (signerContract) {
                    try {
                        logMsg(`🤝 Draw detected — calling emergencyCancelGame(${gameId}) on-chain...`);
                        const tx = await signerContract.emergencyCancelGame(gameId, { gasLimit: 500000 });
                        logMsg(`⏳ emergencyCancelGame tx sent: ${tx.hash}`);
                        await tx.wait();
                        logMsg(`✅ emergencyCancelGame confirmed — both players refunded`);
                        autoRefunded = true;
                    } catch (cancelErr) {
                        logMsg(`❌ emergencyCancelGame failed: ${cancelErr.reason || cancelErr.message}`);
                        logMsg(`❌ Full error: ${JSON.stringify({ code: cancelErr.code, reason: cancelErr.reason, data: cancelErr.data, message: cancelErr.message?.substring(0, 200) })}`);
                        autoRefunded = false;
                    }
                } else {
                    logMsg('❌ signerContract not available — cannot auto-refund draw');
                }
            } else {
                // ── WIN: Generate signature for the winner ──
    ...
    io.to(roomId).emit('gameEnded', {
        ...
        deadline: deadline,
        autoRefunded: autoRefunded, // NEW: tells frontend not to show claim button for draws
        scores: {
            white: whiteScore,
            black: blackScore
        },
...
async function startServer() {
    try {
        ...
        if (process.env.SIGNER_PRIVATE_KEY) {
            const tempSigner = new ethers.Wallet(process.env.SIGNER_PRIVATE_KEY, provider);
            signerContract = moduleContract.connect(tempSigner);
            console.log(`✅ Signer contract ready: ${tempSigner.address}`);
            try {
                ...
                // Check if signer has ADMIN_ROLE (needed for emergencyCancelGame)
                try {
                    const ADMIN_ROLE = ethers.utils.keccak256(ethers.utils.toUtf8Bytes('ADMIN_ROLE'));
                    const hasAdmin = await moduleContract.hasRole(ADMIN_ROLE, tempSigner.address);
                    if (hasAdmin) {
                        console.log(`✅ Signer has ADMIN_ROLE on GameModule — emergencyCancelGame enabled`);
                    } else {
                        console.warn(`⚠️ Signer does NOT have ADMIN_ROLE on GameModule! emergencyCancelGame for draws will fail.`);
                        console.warn(`⚠️ Grant ADMIN_ROLE to ${tempSigner.address} via the contract admin.`);
                    }
                } catch (roleErr) {
                    console.warn(`⚠️ Could not check ADMIN_ROLE: ${roleErr.message}`);
                }
            } catch (err) {
