...
// Ethers.js v5 setup (BSC mainnet)
...
io.on('connection', (socket) => {
    ...
    socket.on('createRoom', (metaData, cb) => {
        ...
        try {
            ...
            // Store wallet address if provided in metaData
            if (metaData && metaData.userAddress) {
                rooms[roomId].player1Address = metaData.userAddress;
            }
    ...
    socket.on('joinRoom', (roomId, cb) => {
        ...
        // Handle cases where roomId might be an object containing the actual roomId and userAddress
        let actualRoomId = roomId;
        let userAddress = null;

        if (typeof roomId === 'object' && roomId !== null) {
            actualRoomId = roomId.roomId;
            userAddress = roomId.userAddress;
        } else if (typeof cb === 'object' && cb !== null && cb.userAddress) { // Fallback if cb is used for data
            userAddress = cb.userAddress;
        }

        const validation = validateInput(actualRoomId, schemas.roomId, 'joinRoom');
        if (!validation.isValid) {
        ...
        try {
            ...
            // Capture joiner's wallet address if provided
            if (userAddress) {
                room.player2Address = userAddress;
            }

            socket.join(actualRoomId);
            socketToRoom[socket.id] = actualRoomId;

            console.log(`✅ ${socket.id} joined room ${actualRoomId} (${room.players.length}/2 players)`);

            // Backward-compatible callback: boolean success
            if (typeof cb === 'function') cb(true);
            // Informational event for newer clients
            safeEmit(socket, 'roomJoined', { roomId: actualRoomId, playerNumber: room.players.length });

            // Start fresh multiplayer game
            room.gameState = null;
            startRoomTimer(actualRoomId);

            safeBroadcast(actualRoomId, 'startGame', {
                freshStart: true,
            ...
            console.log(`🎮 Game started in room ${actualRoomId} with ${room.players.length} players`);

        } catch (error) {
            console.error(`❌ Error joining room ${actualRoomId}:`, error);
    ...
    socket.on('gameEnded', async (gameEndData) => {
        const roomId = socketToRoom[socket.id];
        console.log(`🏁 Explicit game end from ${socket.id} in room ${roomId}:`, gameEndData);
        if (roomId && rooms[roomId]) {
            const room = rooms[roomId];

            // Prevent multiple endings for same room
            ...
            // Handle signature generation for multiplayer claim
            if (gameEndData.winner && room.meta?.gameId) {
                await handleGameSignature(roomId, gameEndData.winner, room.meta.gameId);
            }
        }
    });

    // On-demand signature delivery for claim recovery (similar to Coffee Chess)
    socket.on('requestSignature', ({ gameId, walletAddress: reqWallet }, callback) => {
        ...
        Object.values(rooms).forEach((room) => {
            ...
            // For chinesee, we might need a way to map wallet to player index or just return the relevant signature
            // Assuming player1 is white, player2 is black as in Chess logic
            if (room.signatureWhite || room.signatureBlack) {
                foundSig = room.signatureWhite || room.signatureBlack;
            }
        });
...
// Secure signature generation (Direct copy-paste pattern from Coffee Chess)
async function handleGameSignature(roomId, winnerLabel, gameId) {
    const room = rooms[roomId];
    if (!room) return;

    // Map label to address
    const winnerAddress = (winnerLabel === 'player1') ? room.player1Address : room.player2Address;

    if (!winnerAddress) {
        console.warn(`⚠️ No winner address for ${winnerLabel} in room ${roomId}. Cannot sign.`);
        return;
    }

    console.log(`📝 Generating signature for room ${roomId}: winner="${winnerLabel}" address="${winnerAddress}" gameId=${gameId}`);

    room.signatureWhite = (winnerLabel === 'player1') ? null : room.signatureWhite;
    room.signatureBlack = (winnerLabel === 'player2') ? null : room.signatureBlack;
    ...
    try {
        ...
        if (winnerLabel === 'player1') room.signatureWhite = sig;
        else room.signatureBlack = sig;

        console.log(`✅ Signature created for ${winnerAddress}: ${sig.substring(0, 20)}...`);

        // Broadcast signature to the room
        io.to(roomId).emit('gameSignature', {
            gameId: gameId,
            winner: winnerLabel,
