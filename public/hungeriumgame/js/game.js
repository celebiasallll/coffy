...
class Game {
    ...
    async startGame() {
        ...
        if (this.web3Handler && this.web3Handler.currentAccount) {
            // initializationPromise'ın çözülmesini bekle (timeout ile)
            if (this.web3Handler.initializationPromise) {
                console.log('🎮 Waiting for wallet/contract initialization...');
                try {
                    await Promise.race([
                        this.web3Handler.initializationPromise,
                        new Promise(resolve => setTimeout(resolve, 3000)) // 3 sn limit
                    ]);
                } catch (e) {
                    console.warn("Web3 init bekleme hatası veya timeout:", e);
                }
            }

            try {
                await this.web3Handler.startGameOnContract();
            } catch (err) {
