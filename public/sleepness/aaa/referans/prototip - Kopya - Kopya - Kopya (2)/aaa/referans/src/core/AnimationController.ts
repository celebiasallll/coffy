...
export type AnimState = 'idle' | 'walk' | 'run' | 'jump' | 'runningjump' | 'swim' | 'punch';
...
export class AnimationController {
    ...
    public setState(newState: AnimState, force = false) {
        ...
        if (newState === 'jump' || newState === 'runningjump') {
            ...
            next.enabled = true;
            next.setEffectiveTimeScale(newState === 'jump' ? 0.6 : 1.0); // running jump can be faster
    ...
    public update(dt: number, input: {
        ...
        if (this.isJumping) {
            this.jumpTimer += dt;
            const timeScale = this.currentState === 'jump' ? 0.6 : 1.0;
            const realDuration = this.jumpDuration / timeScale;

            // Physics delay ~0.167s (running) / ~0.556s (standing).
            // Don't land until physics has had a chance to lift the character.
            // Wait at least 0.2s before checking for ground to allow lift-off phase to start
            const physicsDelay = 0.2; 
            const minTime = Math.max(physicsDelay, Math.min(0.4, realDuration * 0.45));
