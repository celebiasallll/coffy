    ...
    if (nearestNPC !== null) {
        if (!isDialogueOpen()) {
            ...
            if (interactEl) {
                ...
                const status = isSat ? "· Kaydedildi" : "· Konuş";
                interactEl.innerHTML = `<span class="kbd">E</span> KÖYLÜ ${status}`;
...
function checkCompletion(npcId: number, player: number) {
    ...
    if (COFFY_COINS >= req) {
        ...
        if (popup) {
            popup.innerHTML = `<div style="color:#ff4444; font-weight:700; letter-spacing:1px; font-family:'Rajdhani',sans-serif;">DAHA FAZLA ☕ COFFY COİN GEREKLİ (${COFFY_COINS}/${req})</div>`;
