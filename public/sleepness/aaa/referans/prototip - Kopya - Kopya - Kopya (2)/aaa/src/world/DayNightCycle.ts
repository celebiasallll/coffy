...
function onDayHotkey(e: KeyboardEvent): void {
  ...
  if (e.code === 'KeyU') {
    ...
    if (import.meta.env.DEV) console.log(`🌫️ Sis: ${_fogActive ? 'AÇIK' : 'KAPALI'}`);
    return;
  }
  ...
  if (import.meta.env.DEV) console.log(`🕐 Zaman → ${labels[e.code]}`);
}
...
export function updateDayNight(
  ...
  if (isDay || isDawn || isDusk) {
    ...
    sun.intensity = rise * 2.8; // Increased for environment brightness
