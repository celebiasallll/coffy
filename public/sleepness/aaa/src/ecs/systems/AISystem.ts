...
export const aiSystem = defineSystem((world: IWorld) => {
    ...
    const inVehicle = InputState.isDriving[playerId] === 1;
    ...
    for (let i = 0; i < enemies.length; i++) {
        ...
        if (staggerTimer > 0) {
            ...
            if (dist < detectionRange && spawnGraceTimer <= 0 && !inVehicle) state = 2; // Chase (ONLY AFTER GRACE PERIOD AND NOT IN VEHICLE)
            else if (state === 2 || (state !== 2 && inVehicle)) {
                state = 0;
            ...
            if (state === 2) { // Chase
                ...
                if (dist > attackRange * 0.95) {
                    ...
                    if (attackTimer <= 0 && Health.current[playerId] > 0 && !inVehicle) {
